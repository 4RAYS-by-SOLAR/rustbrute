...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--output",
        required=False,
        help="Path to save result binaries",
        default=None
    )
    parser.add_argument(
        "--clear-tmp",
        help = "Delete all tmp files during compilation, to prevent memory consumption",
        required=False,
        default=False,
        action="store_true"
    )
...
def main_cli():
    ...
    if args.mode in (
        ...
        if args.mode != "sign_target" and not args.skip_install:
            tc.install()
    ...
    match args.mode:
        ...
        case "download_compile":
            libs = tc.compile_crate(
                ...
                compile_all=args.full_compilation,
                ctx = None if not args.template else CompilationCtx(profile=args.profile, template=template)
            )
            if out_dir:
                for lib in libs:
                    shutil.copy(lib, out_dir)
            if args.clear_tmp:
                for file in glob.glob(os.path.join(tempfile.gettempdir(), f"rustbininfo/*")):
                    if os.path.isdir(file):
                        ...
                        os.remove(file)
                for file in glob.glob(os.path.join(tempfile.gettempdir(), f"rustbinsign/*{Crate.from_depstring(args.crate).name.replace('-','_')}*")):
                    if os.path.isdir(file):
                        shutil.rmtree(file)
                    else:
                        os.remove(file)
                for file in glob.glob(os.path.join(tempfile.gettempdir(), f"rustbinsign/*{Crate.from_depstring(args.crate).name}*")):
                    if os.path.isdir(file):
                        shutil.rmtree(file)
                    else:
                        os.remove(file)
